			...
			innate =>
			{
				innate.AfterYouTakeAction = async (effect, action) =>
				{
					...
					var target = await source.Battle.AskToChooseACreature(source,
						...
						"(Familiar of Restored Spirit) Choose an ally to receive temporary Hit Points.",
			...
			innate =>
			{
				innate.AfterYouTakeAction = async (effect, action) =>
				{
					...
					var target = await source.Battle.AskToChooseACreature(source,
						...
						IllustrationName.ChromaticArmor,
						"(Familiar of Balanced Luck) Choose a creature to have its AC affected.",
						"Choose this one", "Pass");
